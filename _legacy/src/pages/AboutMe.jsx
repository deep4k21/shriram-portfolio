...
export default function AboutMe() {
  ...
  return (
    <div className="flex h-full w-full flex-col gap-5 overflow-hidden rounded-[20px] bg-surface-dark/90 p-section backdrop-blur-[20px]">

      {/* Hero */}
      <div className="shrink-0">
        <h1 className="text-header font-light leading-tight text-neutral-muted">
        ...
        <p className="mt-3 text-body leading-relaxed text-neutral-light">
      ...
      <div className="shrink-0 rounded-2xl bg-surface-darker p-section">
        <p className="text-title font-semibold uppercase tracking-widest text-brand-cyan">
          At a Glance
        </p>
        <p className="mt-2 text-subheader font-light leading-snug text-neutral-light/50">
        ...
        <div className="h-[3rem] overflow-hidden">
          <div className="marquee-up flex flex-col">
            {glanceItems.map((item, i) => (
              <div key={i} className="h-[3rem] shrink-0 leading-[3rem] text-subheader font-light text-neutral-light/50">
                {item}
              </div>
            ))}
            {glanceItems.map((item, i) => (
              <div key={`dup-${i}`} className="h-[3rem] shrink-0 leading-[3rem] text-subheader font-light text-neutral-light/50">
                {item}
              </div>
            ))}
          </div>
      ...
      <div className="flex min-h-0 flex-1 gap-4">
        ...
        {/* My Toolkit */}
        <div className="flex flex-1 flex-col overflow-hidden rounded-2xl bg-surface-darker p-section">
          <h3 className="shrink-0 text-title font-semibold text-brand-cyan">My Toolkit</h3>
          <p className="mt-0.5 shrink-0 text-title text-neutral-muted">
            The tools I use to design, prototype, and bring ideas to life.
          </p>
          <div className="mt-3 grid grid-cols-7 gap-2">
            {myToolkitImages.map((src, i) => (
              <img key={i} src={src} alt="" className="h-icon w-icon cursor-pointer rounded-lg object-contain transition-transform duration-200 hover:scale-125" />
            ))}
          </div>
        </div>
        ...
        {/* AI Workflow */}
        <div className="flex flex-1 flex-col overflow-hidden rounded-2xl bg-surface-darker p-section">
          <h3 className="shrink-0 text-title font-semibold text-brand-cyan">AI Workflow</h3>
          <p className="mt-0.5 shrink-0 text-title text-neutral-muted">
